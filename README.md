...
[![Runner](https://img.shields.io/badge/Runner-0.3.8-0f766e)](scripts/sellersprite-integrated-runner.user.js)
...
| 审计日志 | combined JSON 附带门槛与耗时遥测，也可用 `Copy gate log TSV` / `Copy timing log TSV` 分别复制 |
| 批次控制 | 支持 Generate、Start、Pause、Resume、Clear 和目标合格数停止 |
| 累计目标控制 | 可填写离线回放后的剩余目标，并支持“完成当前行后停止” |
| 可追溯输出 | combined JSON 保留时间、来源、门槛与短路原因；enrichment JSON 提供精简补充字段 |
| 细粒度遥测 | 独立 sidecar 记录导航、Collector 握手、持久化、面板渲染和控制事件 |
| 离线耗时分析 | 严格配对 Gate/Timing TSV，输出阶段分布、残差归因和拒绝成本 |
...
| [sellersprite-integrated-runner.user.js](scripts/sellersprite-integrated-runner.user.js) | 0.3.8 | 管理批次、累计剩余目标、严格门槛和独立细粒度遥测 | [安装 Runner](https://raw.githubusercontent.com/yuanzeli695-byte/sellersprite-userscripts/main/scripts/sellersprite-integrated-runner.user.js) |
| [CONFIGURATION.md](docs/CONFIGURATION.md) | - | 安装顺序、开关、历史台账、迁移和故障排查 | - |
| [P1_TARGET_CONTROL_AND_GRANULAR_TELEMETRY.md](docs/P1_TARGET_CONTROL_AND_GRANULAR_TELEMETRY.md) | - | 累计目标、当前行停止、sidecar 和耗时分析说明 | - |
| [analyze_timing.py](scripts/analyze_timing.py) | - | 离线校验并分析 Gate/Timing TSV | - |
...
| 7 | 浏览器停止预算 | 未填写剩余目标时按 <code>targetQualified</code>；填写后按 <code>remainingTargetAtStart</code> | 浏览器 C/D/E 通过数达到预算后停止队列；最终 A-F 合格仍由离线回放与验收决定 |

> [!NOTE]
> 当前尺寸门槛不判断具体长宽高或重量上限。禁限售、评分、评论、销量、上架时间和变体数仍应在进入 Runner 前完成预筛；现价范围已由 Runner 0.3.8 执行。
...
2. 安装 [SellerSprite Integrated Runner 0.3.8](https://raw.githubusercontent.com/yuanzeli695-byte/sellersprite-userscripts/main/scripts/sellersprite-integrated-runner.user.js)。
...
2. 在 Runner 面板填写 <code>batchName</code>、<code>operator</code> 和原始最终目标 <code>targetQualified</code>；后续批次可填写离线回放得到的 <code>remainingTargetAtStart</code>。
3. 将 ASIN 按“一行一个”粘贴到 <code>QUEUE input</code>。
4. 点击 **Generate** 生成本地批次，再点击 **Start**。
5. Runner 会自动打开队列商品，并按历史、ASIN、流量、尺寸、现价和价格趋势依次判断。
6. 需要中断时点击 **Pause**；要让正在处理的行保存后结束批次，点击 **Stop after current row**。
7. 完成后复制 combined/enrichment JSON、Gate/Timing TSV，以及需要分析时的 granular/control sidecar 导出。

采集图表 tooltip 时应避免频繁切换标签页或移动鼠标，以免扰动可见图表状态。

## 离线耗时分析

将同一运行的 `timing_log_*.tsv` 与 `gate_log_*.tsv` 放在一个目录后执行：

```powershell
npm run analyze:timing -- runs\RUN_ID --format both
```

分析器会在缺列、重复/孤立行、混批、错误时间戳、负耗时或无法解释的空阶段值时失败关闭，并生成 JSON 与 Markdown。输出包含绝对路径、批次名、队列哈希和结果汇总，公开分享前必须脱敏。详细口径见 [P1 文档](docs/P1_TARGET_CONTROL_AND_GRANULAR_TELEMETRY.md)。
...
- Pause 状态不会被正在处理的行覆盖。
- Runner 0.3.8 的累计目标、完成当前行后停止、暂停竞态和 granular sidecar 隔离。
- Python 耗时分析器的配对、残差等式、CLI 输出和失败关闭路径。
...
版本命令只同步脚本的 `@version` 与运行时 `VERSION`：
    ...
    npm run version:integrated -- 0.3.9
...
## 目录结构
    ...
    |-- docs/
    |   |-- CONFIGURATION.md
    |   +-- P1_TARGET_CONTROL_AND_GRANULAR_TELEMETRY.md
    |-- tests/
    |   +-- test_analyze_timing.py
    |-- CHANGELOG.md
    |-- package.json
    +-- README.md
...
- granular sidecar 仍保存在 Amazon 同源 `localStorage`；其导出可能包含批次、队列哈希和耗时证据，公开前应脱敏。
