...
本文对应当前版本：Integrated Runner `0.3.8`、Traffic Collector `0.4.6`。
...
3. 再安装 [Integrated Runner 0.3.8](https://raw.githubusercontent.com/yuanzeli695-byte/sellersprite-userscripts/main/scripts/sellersprite-integrated-runner.user.js)。
...
| `targetQualified` | 完整运行最初请求的严格合格目标 | 按任务填写，默认 `20`；后续批次不要改写 |
| `remainingTargetAtStart` | 本批浏览器停止预算 | 可留空；后续批次填写“原目标 - 最新离线回放合格数” |
...
- `Stop after current row`：让正在处理的行保存后结束批次；不会取消 Collector 或生成半条结果。
...
| `ENABLE_TIER0_GRANULAR_TELEMETRY` | `true` | 写入独立 `sellerSpriteGranularTelemetry/v1` sidecar |
| `ENABLE_P1_CUMULATIVE_TARGET_CONTROL` | `true` | 启用剩余目标和“完成当前行后停止”控制 |
...
ssIntegratedRunner:v0.3:granular:v1:<queueHash>
```
...
从 `Runner 0.3.7 + Collector 0.4.6` 升级时：

1. 先暂停或完成旧批次，并复制需要保留的 JSON。
2. 保持 Collector `0.4.6`，更新 Runner 到 `0.3.8`。
3. 停用旧脚本，刷新新的 Amazon 商品页。
4. 新任务重新 Generate；后续批次只在离线回放后填写 `remainingTargetAtStart`。

0.3.8 不改变 A-F 门槛、Collector 协议、Gate/Timing TSV 或严格历史指纹。新增 target-control 字段和 granular sidecar 均为附加证据，不能直接授予最终合格。回滚点为 Runner `0.3.7` + Collector `0.4.6`。

## 7. 隐私与故障排查

- 两个脚本不要求填写 API Key、密码或导出 Cookie，也不使用 `fetch`、XHR 或 WebSocket 上报数据；运行时仍依赖 Chrome 中已登录的 Amazon/SellerSprite 会话。
- Runner 的批次、历史和遥测只保存在当前浏览器的 Amazon 同源 `localStorage`；不要在 `operator` 或 `batchName` 中填写敏感信息。granular 导出还包含批次名、队列哈希和耗时证据，公开前必须脱敏。
